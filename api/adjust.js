...
function generateOAuthHeader(method, baseUrl, queryParams, config) {
  ...
  return "OAuth " + Object.keys(headerParams).map((k) => `${k}="${encodeRFC3986(headerParams[k])}"`).join(", ");
}
...
export default async function handler(req, res) {
  ...
  const { locationId, locationName, accountId, subsidiary, items, memo } = req.body;

  if (!locationId || !items || items.length === 0) {
    return res.status(400).json({ error: "Missing locationId or items" });
  }

  // Build the inventory adjustment payload
  const adjustmentBody = {
    subsidiary: { refName: subsidiary || "Great Lakes Work Wear" },
    account: accountId ? { id: accountId } : { refName: "60050 Inventory Adjustment" },
    ...
    inventory: {
      items: items.map((item, idx) => {
        ...
        // Add inventory detail with bin if bin_id exists
        if (item.bin_id) {
  ...
  try {
    const nsResponse = await fetch(baseUrl, {
      ...
      headers: {
        ...
        Prefer: "respond-async, resultwait=30",
      },
      body: JSON.stringify(adjustmentBody),
    });

    // NetSuite returns 204 with Location header on success
    if (nsResponse.status === 204 || nsResponse.status === 201) {
      const locationHeader = nsResponse.headers.get("Location") || "";
      // Extract the record ID from the location header
      const idMatch = locationHeader.match(/\/(\d+)$/);
      const recordId = idMatch ? idMatch[1] : null;
      const recordUrl = `https://${config.accountId}.app.netsuite.com/app/accounting/transactions/invadjst.nl?id=${recordId}`;

      return res.status(200).json({
        success: true,
        recordId,
        recordUrl,
        message: `Inventory adjustment created successfully${recordId ? ` (ID: ${recordId})` : ""}.`,
      });
    }
