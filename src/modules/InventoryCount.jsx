...
const SESSION_KEY = "glww_inventory_count";
...
export default function InventoryCount({ onBack }) {
  ...
  const [locationBinMap, setLocationBinMap] = useState(saved?.locationBinMap || {});
  ...
  const pullInventory = async () => {
    ...
    try {
      ...
      const noBinItemsResult = await suiteql(`
        SELECT
          item.id AS internalid,
          item.itemid AS sku,
          item.displayname AS itemname,
      ...
      `);
      setNoBinItems(noBinItemsResult);
  ...
  const restartCount = () => {
    ...
    setFlash(null); setFilter(""); clearSession(SESSION_KEY);
  };
    ...
    return (
      <div style={S.root}>
        ...
        <div style={S.hdr}>
          ...
          <button style={S.btnSm} onClick={() => { setLookupResults([]); setError(null); setPhase("setup"); }}>← Back</button>
        </div>
      ...
      <div style={S.root} onClick={() => { if (!currentBin) binRef.current?.focus(); else scanRef.current?.focus(); }}>
        ...
        <div style={{ padding: "10px 16px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
            <button style={{ ...S.btnSec, padding: "10px 14px", flex: 1 }} onClick={() => { clearSession(SESSION_KEY); setPhase("setup"); }}>← Setup</button>
